    ...
    def evaluate(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, (images, labels) in enumerate(self.data):
                ...
                # compute output
                recon_x, mu, logvar = self.model(images)
                self.eval_loss += self.criterion(recon_x, images, mu, logvar).item()
                ...
                if batch_idx == 0:
                    ...
                    img_nm = os.path.join(self.config.data['generated'], 'reconstruction_' + str(epoch) + '.png')
